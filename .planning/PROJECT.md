...
- [ ] Multi-format document input (PDF, PNG, JPG)
- [ ] LLM-powered extraction using vision-capable models (start with one provider, abstraction for adding more)
- [ ] Schema-driven output using Zod — every extraction validated against a schema, malformed output retried or flagged
- [ ] Confidence scoring per extracted field
- [ ] Built-in templates for 2-3 document types (invoice, receipt, + one more) with extensible template system
- [ ] BYOK — users provide their own LLM API key
- [ ] Core engine publishable as npm package with clean, well-documented API
...
- [ ] Monorepo structure with shared core (Turborepo)
...
| Core + web app priority over CLI | These are the most visible for portfolio. CLI is functional but lower polish | — Pending |
